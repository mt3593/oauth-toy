...
if [ -n "$PIDS" ]
then
  echo "Oauth-toy is already running in process $PIDS";
  exit 1
fi
...
LOG_FILE=${LOGGING_PATH}/oauth-toy.out
ERR_FILE=${LOGGING_PATH}/oauth-toy.err
...
echo "Giving Oauth-toy $waitTimeout seconds to start successfully"
...
do
  ...
  then
    echo "Oauth-toy didn't start within $waitTimeout seconds."
    PIDS=$(pgrep java -lf | grep $APP_NAME | cut -d" " -f1);
    if [ -n "$PIDS" ]
	then
	  echo "Killing $PIDS";
...
if [ $retVal -eq 1 ]
then
  echo "Starting Oauth-toy failed"
else
  echo "Starting Oauth-toy succeeded"
fi
